...
NprtResultHardwareIdentity nprt_sec_read_hardware_identity(void) {
  ...
  r.ok = true;
  if (!read_cmd_line("ioreg -rd1 -c IOPlatformExpertDevice | awk -F\\\" '/IOPlatformUUID/{print $(NF-1); exit}'", r.value.board_uuid, sizeof(r.value.board_uuid))) {
    strncpy(r.value.board_uuid, "board-unknown", sizeof(r.value.board_uuid) - 1);
  }
  if (!read_cmd_line("system_profiler SPHardwareDataType | awk -F': ' '/Hardware UUID/{print $2; exit}'", r.value.bios_uuid, sizeof(r.value.bios_uuid))) {
    strncpy(r.value.bios_uuid, "bios-unknown", sizeof(r.value.bios_uuid) - 1);
  }
  if (!read_cmd_line("networksetup -listallhardwareports 2>/dev/null | awk '/Ethernet Address/{print $3; exit}'", r.value.mac_address, sizeof(r.value.mac_address))) {
    strncpy(r.value.mac_address, "mac-unknown", sizeof(r.value.mac_address) - 1);
  }
  if (!read_cmd_line("diskutil info / | awk -F': ' '/Disk \\/ Partition UUID/{print $2; exit}'", r.value.disk_serial, sizeof(r.value.disk_serial))) {
...
NprtResultVmSignals nprt_sec_detect_vm_signals(void) {
  ...
  char line[256] = {0};
  r.ok = true;
  r.value.cpuid_hypervisor = read_cmd_line("sysctl -n machdep.cpu.features 2>/dev/null | grep -qi HYPERVISOR && echo yes", line, sizeof(line));
  r.value.file_registry_traits = read_cmd_line("system_profiler SPHardwareDataType | grep -Ei 'VirtualBox|VMware|QEMU|Parallels' | head -n1", line, sizeof(line));
  r.value.device_driver_traits = read_cmd_line("kextstat 2>/dev/null | grep -Ei 'vbox|vmware' | head -n1", line, sizeof(line));
